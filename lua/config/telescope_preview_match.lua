...
local function collect_local_variables(node, bufnr)
    return collect_nodes(node, bufnr, function(n)
    ...
    pickers.new({}, {
        ...
        previewer = previewers.new_buffer_previewer({
            define_preview = function(self, entry, status)
                if not entry.value then return end
            ...
            actions.select_default:replace(function()
                ...
                if selection and selection.value then
                    vim.api.nvim_win_set_cursor(0, {
                        selection.value.row, selection.value.col - 1
                    })
                end
            end)
            return true
...
function M.register_commands()
    -- Command: LocalVars
    vim.api.nvim_create_user_command("LocalVars", function()
    ...
    vim.api.nvim_create_user_command("FileSelfRefs", function()
        local bufnr = vim.api.nvim_get_current_buf()
        local parser = get_parser(bufnr)
        if not parser then return end
